...
We plan to release our benchmarks in this repo: <https://github.com/getkono/kynos-bench>, which also defines the measurement methodology — what is measured, how, and why each number is worth tracking. Until those numbers are published, treat the comparative half of this answer as unestablished; the structural claim above it is checked by `mise run containment:check`, which holds the emitted document, the schema registry, the document validators and the JSON Schema interpreter to the files [`docs/testing.md`](docs/testing.md#the-off-path-proof) allows them under `crates/kynos/src/`, beside a witness that pins the fields the dispatch table stores. That is a naming rule and a ratchet rather than a proof of unreachability, and [`docs/testing.md`](docs/testing.md#the-off-path-proof) says what each half does and does not carry.
