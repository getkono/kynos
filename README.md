...
We plan to release our benchmarks in this repo: <https://github.com/getkono/kynos-bench>, which also defines the measurement methodology — what is measured, how, and why each number is worth tracking. Until those numbers are published, treat the comparative half of this answer as unestablished; the structural claim above it is checked by `mise run containment:check`, which holds the emitted document, the schema registry, the document validators and the JSON Schema interpreter to the sites [`docs/testing.md`](docs/testing.md#the-off-path-proof) allows them, beside a witness that pins every field a request reaches through the dispatch table.
