...
| [`flatten.rs`](../crates/kynos/tests/flatten.rs) | that a flattened field's description accepts the JSON its type writes, checked by serializing a value and validating it against the emitted schema with a real JSON Schema validator: a flattened struct, an open map beside the parent's own properties and beside a flattened struct, an open map inside a tagged variant, the key constraint an open flatten drops, and an object `deny_unknown_fields` closes beside a flattened struct, whose refusals are held to serde's. Behind `macros` and `test-util`, which carries the validator |
