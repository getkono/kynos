...
| [`flatten.rs`](../crates/kynos/tests/flatten.rs) | that a flattened field's description accepts the JSON its type writes, checked by serializing a value and validating it against the emitted schema with a real JSON Schema validator: a flattened struct, an open map beside the parent's own properties and beside a flattened struct, an open map inside a tagged variant, the key constraint an open flatten drops, a flattened `Problem` beside a member of the parent's own and beside an open map, and an open `Unchecked` payload beside the parent's own properties. Behind `macros` and `test-util`, which carries the validator |
