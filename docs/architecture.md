...
Kynos is built on tokio, and that is a decision rather than a default.
...
- tokio is a hard dependency, not a generic parameter. No runtime abstraction
  traits exist in core.
- tokio types appear in zero handler-facing signatures. Replacing the runtime
  must be a change to this crate, never a migration for anyone using it.
- The runtime coupling surface is exactly five points, and they live in one
  module behind one internal trait that is not public API: the accept loop and
  listener, connection socket read and write (the `hyper::rt` implementation),
  `spawn`, timers (request timeout, keepalive, shutdown grace), and the shutdown
  signal.
...
- io_uring is a post-1.0 experiment, contingent on a measured win in a workload
  Kynos actually targets. If it ever ships it is an alternate connection driver,
  not an alternate framework, and it is not a design constraint today.
...
The diagnostic is therefore syscalls per request, measured before any design
work is contemplated. At roughly four syscalls per request there is nothing left
to win, and the exercise is over.
