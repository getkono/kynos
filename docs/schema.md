...
internally tagged enum with a newtype variant. A variant serde skips both ways
...
| 17 | A described field or variant whose wire form serde's `with`, `serialize_with` or `deserialize_with` decides is refused; every newtype-struct member counts as described, whatever serde skips, and so does every other unnamed member serde does not skip both ways; a skipped named field, an unnamed member serde skips both ways outside a newtype struct, or any field of a variant serde skips both ways is left alone, and so is `serialize_with` on a variant serde reads and never writes or on its fields | the derive's ledger in [`derive/tests.rs`](../crates/kynos-macros/src/derive/tests.rs), and `tests/ui/macros/schema_serialize_with.rs` for the wording |
