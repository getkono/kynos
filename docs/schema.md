...
content keys serde names. An externally tagged enum, which serde would read by
its variant key, is not `Flatten` at all. `Box<T>` and `Arc<T>` carry the
