...
an open map. The schema leaves that field out, and only a hoisted
`unevaluatedProperties` would refuse it. The derive bounds an open field
beside such a field by
[`AdmitsAny`](https://docs.rs/kynos/latest/kynos/schema/trait.AdmitsAny.html),
which `Unchecked` implements and a map does not, because whether the field's
type hoists anything is visible to the compiler and not to the derive.
...
| 30 | `#[serde(skip_deserializing)]` alone on a named field is refused in an object serde writes that `deny_unknown_fields` closes under rule 32, and beside a described `#[schema(open)]` flattened field unless that field's type implements `AdmitsAny`, since the closing keyword or the hoisted `unevaluatedProperties` refuses the member serde writes and the schema leaves out; an open `Unchecked` map implements it and hoists nothing, so the field is admitted beside it; a struct, or an internally tagged enum with a struct variant serde writes, holding such a field does not claim `Flatten`; a field serde skips both ways, an open map serde never reads and a variant serde never writes are left alone | the derive's ledger, `AdmitsAny`-witness and `Flatten`-claim rows in [`derive/tests.rs`](../crates/kynos-macros/src/derive/tests.rs); `a_field_serde_never_reads_beside_an_open_unchecked_field_is_admitted` in [`tests/flatten.rs`](../crates/kynos/tests/flatten.rs), against the `jsonschema` validator; and `tests/ui/macros/schema_field_skipped_on_read_denying_unknown_fields.rs` for the wording of the closed object's refusal, and `tests/ui/macros/schema_field_skipped_on_read_beside_open_map.rs` for the `AdmitsAny` bound's |
