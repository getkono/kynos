...
| correctness | An endpoint-scoped interceptor reaching a router through an `Endpoints` collection is checked | — | `absent`, and recorded in [`middleware.md`](middleware.md#what-the-compile-time-check-still-cannot-see): a collection erases what its members carry, and a build-time backstop would need a declaration method on the public `Endpoint` trait — the `contribution` method the design refuses. Use `routes!`. |
...
| reliability | A streamed response body is bounded in time, both between frames and in total | [`tests/limits.rs`](../crates/kynos/tests/limits.rs) over a stalled stream, a steady one as the control, and a deadline ending a stream that is still producing | `enforced` |
...
| correctness | A language tag is well-formed, and is not checked for existence | [`response/language/tests.rs`](../crates/kynos/src/response/language/tests.rs)'s `a_tag_naming_no_real_language_is_still_well_formed` | `by-design`, and argued in [`architecture.md`](architecture.md): the IANA Language Subtag Registry is a database only sampling can verify, and serving `zz-Qaaa-QM` the default costs a client what a request for `ja` already costs it |
| dx | An offer discovered at run time can state its language, but cannot enumerate it | [`tests/description.rs`](../crates/kynos/tests/description.rs)'s `a_catalogue_no_const_can_name_still_declares_the_field_without_its_offer` | `partial`; `Languages::TAGS` is a `const` because `Describe` and `Responses` take no `self`, so a catalogue loaded from disk describes `Content-Language` as an unconstrained string. Closing this needs a second, weaker path in `Localized`, which would leave a reader unable to tell from a signature which one an operation used |
...
| compatibility | A pull request reports whether it broke the public API, across every feature | `mise run semver:check`, dedicated CI job, comparing the workspace at `--all-features` against the last published version | `partial`: the verdict goes to the job summary rather than failing the pull request, because it reports a break against the *published* baseline and 0.1.0 is still that baseline — `Router::group`/`nest`/`merge` and `Timeout`'s private field are the breaks the pending 0.2.0 release exists to declare, so every pull request would carry a red mark meaning "the release has not happened yet". The job still fails when the tool produces no verdict at all, which is the fail-open hole the release-time copy has. When 0.2.0 publishes, the baseline moves past both and requiring this becomes a ruleset change |
...
| reliability | Commits follow Conventional Commits, merge commits exempt | `convco`, twice over: the `conventional-commit` `commit-msg` step runs `mise run commits:message` over the one message being written and exempts a merge on the presence of the `MERGE_HEAD` *file*, which git writes before it runs that hook — a file test and not `rev-parse`, since a branch or tag of that name resolves while no merge is in progress; `mise run commits:check` and the `commits` CI job run `convco check` over a range, where the exemption is convco's own parent-count filter. `mise run commits:test` runs *both* halves over the same commits, since a divergence between them fails neither and the range half is the one that moves under maintenance: a `.convco` changes what the range half accepts and leaves the hook half untouched. A `[tools]` bump of convco can reach either, which is why the suite asserts on convco's own rejection wording rather than on a bare non-zero exit — a bump that reworded it fails there instead of quietly turning every rejection assertion into a test that the gate exited | `enforced`, with one case out of reach: amending or rewording an *existing* merge commit runs the hook with `MERGE_HEAD` already gone over a commit that still has two parents, so the hook rejects what the range form exempts. Nothing at commit-msg time distinguishes it from committing on top of a merge, and `--no-verify` is the escape. That residual is the only one, and `commits:test` pins it in both directions, so closing it or widening it fails this row rather than leaving it silently wrong |
