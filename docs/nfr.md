...
| security | `Strict-Transport-Security` is never sent over a transport known not to be secure | [`security_headers/tests.rs`](../crates/kynos/src/middleware/security_headers/tests.rs) over the configured-and-secure and configured-and-not pair | `enforced` |
| correctness | Every name the security-header group declares is one some configuration sends, and the reverse | the same file, comparing `NAMES` against a maximal configuration | `enforced` |
