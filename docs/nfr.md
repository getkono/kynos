...
| reliability | Commits follow Conventional Commits, merge commits exempt | `convco`, twice over: the `conventional-commit` `commit-msg` step runs `mise run commits:message` over the one message being written and exempts a merge on `MERGE_HEAD`, which git writes before it runs that hook; `mise run commits:check` and the `commits` CI job run `convco check` over a range, where the exemption is convco's own parent-count filter. `mise run commits:test` holds the two to the same answer, since a divergence between them fails neither | `enforced`, with one case out of reach: amending or rewording an *existing* merge commit runs the hook with `MERGE_HEAD` already gone over a commit that still has two parents, so the hook rejects what the range form exempts. Nothing at commit-msg time distinguishes it from committing on top of a merge, and `--no-verify` is the escape |
