...
| `cargo-llvm-lines` | The codegen-delta kind in [`performance.md`](performance.md#the-taxonomy), which is what a type-level surface owes | Closed. `mise run cost:features` runs it at each feature over [`cost/fixture.rs`](../crates/kynos/cost/fixture.rs), in the dev profile because a fat-LTO build deletes the monomorphizations this counts, and attributes each delta to the functions that moved. The `.text` half of the same sweep answers the binary-delta row beside it. Neither sets a ceiling — see [Thresholds](#thresholds) |
